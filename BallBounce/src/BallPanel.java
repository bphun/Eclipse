...
public class BallPanel extends Canvas {
	...
	public void loop() {
		...
		while(true) {
			...
			try {
				// Thread.sleep(getFpsDelay(MAX_FRAME_RATE));
				Thread.sleep(12);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void render() {
		...
		// Render Foreground (text, etc)
  		...
  		if (tempArrow != null) {
  			...
  			this.g2.drawString(String.format("%.2f px/s", tempArrow.getLength() * ARROW_WIDTH, 2), (tempArrow.getX2() + tempArrow.getX1()) / 2, (tempArrow.getY1() + tempArrow.getY2()) / 2);
  		}
