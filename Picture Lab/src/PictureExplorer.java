...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener
{
  ...
  //label to let user know what type of selection is taking place
  private JLabel selectTypeLabel=new JLabel("Selection Type:");
  private JTextField selectionType =new JTextField( " none  ");
  ...
  private void setUpSelectType() {
	...
	JMenuItem rectSelect = new JMenuItem("Rectangle");
	JMenuItem lineSelect = new JMenuItem("Line");
	selectType.add(lineSelect);
	selectType.add(rectSelect);
	rectSelect.addActionListener(this);
	lineSelect.addActionListener(this);
	menuBar.add(selectType);
}
...
/**
  ...
  private JPanel createColorInfoPanel(Font labelFont)
  {
    ...
    selectionType.setEditable(false);
    colorInfoPanel.add(this.selectTypeLabel);
    colorInfoPanel.add(this.selectionType);
...
//    if((startPoint == null || endPoint == null)&&selectedRect==null && selectedLine==null) {
...
//    	System.out.println("drawing rect: "+x1+" , "+y1+" w: "+(x2-x1)+" h: "+(y2-y1));
...
//    }
  ...
  {
//    System.out.println("dragging "+firstClick +" start: "+startPoint+" end: "+endPoint);
...
//	  this.repaint();
  ...
  {
	  // This is stuff that Mr. Hanson was adding to add a selecting line or rectangle...
//	  if(this.selectingLine) {
...
//	  firstClick = true;
  ...
  {
    if(a.getActionCommand().equals("Line")) {
    	this.selectingLine=true;
    	this.selectingRegion=false;
    	this.firstClick=true;
    	this.selectionType.setText(" Line  ");    
    }
    if(a.getActionCommand().equals(" Rect  ")) {
    	this.selectingLine=false;
